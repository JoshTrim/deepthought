...
class STT:
    ...
    def stt(self, audio_file_path):
        return stt_wav(self.service_directory, audio_file_path)
...
def export_audio_to_wav_ffmpeg(audio:bytearray, mime_type:str) -> str:
    ...
    input_path = os.path.join(temp_dir, f"input_{datetime.now().strftime('%Y%m%d%H%M%S%f')}.wav")
    ...
    if mime_type == "audio/raw":
        ...
        ffpmeg.input(input_path).output(output_path, acodec="pcm_s16le", ac=1, ar="16k", loglevel="panic").run()
...
stt = STT({"service_directory": os.getcwd()})
file = Path("test.m4a")

r = stt.stt(file.absolute())
print(r)
